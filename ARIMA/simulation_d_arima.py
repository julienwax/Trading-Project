...
import matplotlib.pyplot as plt
from arima_model import prediction,order
...
class Trading_bot():
    ...
    def buy_sell(self,total_avail,pos,logf):
        ...
        next_price = prediction(date,self.chartdict['INI'][self.ticker])
        p=self.get_latest_price(self.ticker)
        if next_price > p:
            p=self.get_latest_price(self.ticker)
            size=total_avail//p
            return 'BUY',self.ticker,p,size
        elif (pos!=0 and next_price < p):
            p=self.get_latest_price(self.ticker)
            size=pos
...
if __name__ == "__main__":
    ...
    ROI = bot.pool("Ordre_courant",somme_ini,False); print(ROI)
    ...
    df1.plot(c='darkblue')
    plt.title("Evolution du portefeuille, ROI = {:.2f}".format(ROI) + "  |  (p,d,q) = "+str(order)) ;plt.show()
