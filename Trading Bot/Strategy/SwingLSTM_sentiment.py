...
def sentiment_signal(df):
    sentiment_df = pd.read_csv("Data/Sentiment.csv")
    sentiment_df.rename(columns={'datetime': 'Date'}, inplace=True)
    sentiment_df.rename(columns={'score': 'Sentimentsignal'}, inplace=True)
...
def total_signal(df):
    ...
    for row in range(0, len(df)):
        TotalSignal[row] = 0
        if df.LSTMsignal[row]==1 and df.EMAsignal[row]==1 and df.Sentimentsignal[row]<-0.5:
            TotalSignal[row]=1
        if df.LSTMsignal[row]==2 and df.EMAsignal[row]==2 and df.Sentimentsignal[row]>0.5:
            TotalSignal[row]=2
...
def apply_save(filename,backtest=False):
    ...
    if backtest:
        ...
        df.to_csv("Data/"+filename+"Sentiment.csv", index=False)
