### LSTM model for S&P 500 ETF with optimized parameters###
import os
import numpy as np
import pandas as pd 
...
df = pd.read_csv('../data_lstm/SPY_5MINS_6M_2024-06-25.csv') # (9461,6)
...
training_set = df.iloc[:int(splitlimit*len(df)), 4:5].values # (7568, 1)
test_set = df.iloc[int(splitlimit*len(df)):, 4:5].values # (1892, 1)

training_set_vol = df.iloc[:int(splitlimit*len(df)), 5:6].values  # (7568, 1)
test_set_vol = df.iloc[int(splitlimit*len(df)):, 5:6].values # (1892, 1)
...
X_p,Y = make_vectors(training_set_scaled) # (504, 15, 1) both
X_vol,_ = make_vectors(training_set_vol_scaled) # (504, 15, 1) both
X_test_p,Y_test = make_vectors(test_set_scaled) # (126, 15, 1) both
X_test_vol,_ = make_vectors(test_set_vol_scaled) # (126, 15, 1) both

X = np.concatenate((X_p,X_vol),axis=2)  # (504, 15, 2) | Y and Y_test has size (504, 15, 1) and (126, 15, 1) respectively
X_test= np.concatenate((X_test_p,X_test_vol),axis=2) # (126, 15, 2)
    ...
    def on_epoch_end(self,epoch, logs=None):
        ...
        if callback:
            ...
            if input("Do you want to save the model ? (y/n)") == "y":
                model.save(f"../trained_model/SPYmodel_5min_{seq_length}_{indicators}.keras")
                print("Model saved")
