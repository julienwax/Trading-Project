...
count=0
epsilon=1e-5
for p in range(0,len(training_set)-seq_length,seq_length):
    for l in range(seq_length):
        X[count][l][0]=epsilon+(epsilon+training_set[p+l+1]-training_set[p+l])/(epsilon+training_set[p+l])
        Y[count][l][0]=epsilon+(epsilon+training_set[p+l+2]-training_set[p+l+1])/(epsilon+training_set[p+l+1])
    count += 1
count=0
for p in range(0,len(test_set)-seq_length,seq_length):
    for l in range(seq_length):
        X_test[count][l][0]=epsilon+(epsilon+test_set[p+l+1]-test_set[p+l])/(epsilon+test_set[p+l])
        Y_test[count][l][0]=epsilon+(epsilon+test_set[p+l+2]-test_set[p+l+1])/(epsilon+test_set[p+l+1])
    count += 1
...
def scale_data(X,scaler):
    ...
    for p in range(0,len(data_scaled)-seq_length,seq_length):
        for l in range(seq_length):
            X[count][l][0]=data_scaled[p+l][0]
        count += 1
    return X

X = scale_data(X,scaler)
y = scale_data(Y,scaler)
X_test = scale_data(X_test,scaler_test)
Y_test = scale_data(Y_test,scaler_test)
...
class MyCallback(Callback):
    def on_epoch_end(self,epoch, logs=None):
        if epoch%10==0:
            ...
            pred_test = model.predict(X[50].reshape(1,seq_length,vocab_size),batch_size=1)
            ...
            ax2.plot(list(range(seq_length)),Y[50][:],label="data_test",c = 'darkblue')
...
model.compile(tf.keras.optimizers.Adam(learning_rate=learning_rate),tf.keras.losses.MeanSquaredError(),metrics=['accuracy'])
model.fit(X,Y,batch_size=1,epochs=600, validation_split=0.1,callbacks=[MyCallback()])
