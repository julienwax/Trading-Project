...
access_token='XXXXX'# access token from OANDA account
accountID="XXXXX" # account ID from Oanda
lotsize = 3000  # lot size for trading
...
def get_candles_frame(n):
    ...
    for candle in candles:
        ...
        dfstream.loc[i, ['Volume']] = float(str(candle.volume))
        i=i+1
...
def fitting_job():
    ...
    class MyStrat(Strategy):
        mysize = 3000 # size for the optimization
        slcoef = 1.1
        TPSLRatio = 1.5
...
scheduler.add_job(trading_job, 'cron', day_of_week='mon-fri', hour='07-18', minute='1, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56', timezone='Europe/Paris', misfire_grace_time=15)
scheduler.start()