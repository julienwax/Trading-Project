...
class trading_bot():
    ...
    def buy_sell(self,total_avail,pos,logf):
        ...
        if (self.is_market_open('AAPL')):
            ...
            elif (pos!=0 and p > 1.05*prix_achat) or (pos != 0 and p < 0.95*prix_achat):
                size=pos
                #print("  J'aimerais vendre "+str(size)+" actions a  " + str(p) +" $")
                prix_vente = p
                return 'SELL','AAPL',p,size
            elif p < 1.01*prix_vente:
                size = total_avail//p
                prix_achat = p
                return 'BUY','AAPL',p,size
            elif latence > 5000 and pos!=0 and p>1.01*prix_achat:
                #print(" la latence est élevée !")
                size = pos
                prix_vente = p
                return 'SELL','AAPL',p,size 
            elif latence > 5000 and p<0.99*prix_vente:
                #print(" la latence est élévée !")
                size = total_avail//p
                prix_achat = p
                return 'BUY','AAPL',p,size
            else:
