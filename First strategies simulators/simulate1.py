...
class trading_bot():
    ...
    #-------------------------------------------------------------------------------#
    # à vous d'ajouter votre stratégie ici
    ...
    #-------------------------------------------------------------------------------#
        ...
        # passé 30 jours.
            offset = pd.DateOffset(minutes=60)  # Fenêtre de 1 heure
            start = date - offset
            end = date - pd.DateOffset(minutes=1)  # nous connaissons le prix uniquement jusqu'à la dernière minute
            df = self.chartdict['PT1M']['AAPL']
            df_range = df[start:end]
            price = pd.Series(list(df_range[:]), index=range(0, len(df_range), 1))

            m_mob = price.rolling(window=24*3).mean().iloc[-1]

            m_gli = price.rolling(window=2).mean().iloc[-1]
